...
function PredictionRow({ pool, fencer, closed, onRefresh, reportDirty }) {
  ...
  useEffect(()=>{reportDirty(fencer.id,dirty);return()=>reportDirty(fencer.id,false)},[dirty,fencer.id,reportDirty]);
  ...
  return <tr id={`pool-${fencer.id}`} className={closed ? 'pool-table-row is-closed' : 'pool-table-row'}>
    ...
    <td data-label="Nationalité · classement"><div className="pool-fencer-info">
...
function PoolList({ competitionId, user, onDirtyChange }) {
  ...
  useEffect(()=>{onDirtyChange?.(dirty);const leave=e=>{if(dirty){e.preventDefault();e.returnValue=''}};window.addEventListener('beforeunload',leave);return()=>{onDirtyChange?.(false);window.removeEventListener('beforeunload',leave)}},[dirty,onDirtyChange]);
